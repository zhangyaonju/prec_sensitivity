...
if ind_process_dlm:
    lai_file = root+'/Project/water_availability/data/MOD13C2_'+varused+'_2000_2020_snow_cloud.nc'

    with Dataset(lai_file,'r') as fin:   # starts from 2001/1
        lai = fin.variables[varused][:][:,:,11:(yr_ind-1189)]

    prec_file = root+'/Data/CRU/cru_ts4.04.1901.2020.pre.dat.nc'
    clou_file = root+'/Data/CRU/cru_ts4.04.1901.2020.cld.dat.nc'
    temp_file = root+'/Data/CRU/cru_ts4.04.1901.2020.tmp.dat.nc'
    with Dataset(prec_file,'r') as fin:   # starts from 1999/1
        prec = fin.variables["pre"][:][1176:yr_ind,:,:]
    with Dataset(clou_file,'r') as fin:   # starts from 1999/1
        clou = fin.variables["cld"][:][1176:yr_ind,:,:]
    with Dataset(temp_file,'r') as fin:   # starts from 1999/1
        temp = fin.variables["tmp"][:][1176:yr_ind,:,:]
    '''
    temp = readcruncep("tair")
    prec = readcruncep("prec")
    clou = readcruncep("solr")
    '''
    laina = np.where(lai< -0.1, np.nan,lai)
    lai = np.transpose(lai,(2,0,1))
    print(lai.shape)
    print(prec.shape)
    fill_value = -999
    ...
    outfile = root+'/Project/water_availability/analysis/DLM_MODIS/multi_modis'+st+'.'+varused+'.DLM.CRU.spinup'+str(spin_up_years)+'.rep'+str(spin_up_rep)+'.delta'+str(delta)+'.nc'
