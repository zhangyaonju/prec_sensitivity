...
nvar<-c("gimms","multi_gimms")[as.numeric(myargs4)]

## deseasonlized and detrended variables are included
scenario<-paste0("spinup",spinup,".rep",spinrep,".delta",delta)
ncin<-nc_open(paste0("./analysis/DLM_LAI/",nvar,".DLM.CRU.",scenario,".nc"))
predictmean<-ncvar_get(ncin,varid="Predictmean")
Xpre<-ncvar_get(ncin,varid="Regrevariable")
nc_close(ncin)
xdim = dim(Xpre)[4]

y_pre_ndvi<-predictmean[,,1:(414+spinup*12*spinrep),3]
y_prec_12<-predictmean[,,1:(414+spinup*12*spinrep),4]
rm(predictmean)
...
ndvi_sens_mean_trend<-parApply(cl,y_pre_ndvi[,,121:534],c(1,2),get_mean_trend)
prec_sens_mean_trend<-parApply(cl,y_prec_12[,,121:534],c(1,2),get_mean_trend)

save(ndvi_sens_mean_trend,prec_sens_mean_trend,file=paste0("./analysis/DLM_LAI/",nvar,"_CRU_coef_sensi_mean_trend_",scenario,".RData"))
