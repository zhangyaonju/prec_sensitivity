...
lai_file<-list.files("./Data/MsTMIP_v2/LAI/",full.names = T)[args]
...
for (i in 1:11){
  ...
  dseason_fpar<-parApply(cl,fpar[,,(i*120-119):(i*120)],c(1,2),deseason)
  ...
  comb1 <- abind(dseason_fpar,cm,along=1)
  all_periods_stat[i,,,]<-parApply(cl,comb1,c(2,3),MLR_model4)
}

save(all_periods_stat,file=paste0('./Project/water_availability/analysis/MsTMIP_LAI_v2/partial_mlr_LAI_MsTMIP_',model_name,'_best_model.RData'))
